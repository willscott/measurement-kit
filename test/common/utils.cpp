...
TEST_CASE("percentile() works as expected with zero length vector") {
    std::vector<double> v;
    REQUIRE_THROWS(mk::percentile(v, 0.1));
}
...
TEST_CASE("percentile() works with one-element vectors") {
    ...
    REQUIRE(mk::percentile(v, 0.5) == 17.0);
    REQUIRE(mk::percentile(v, 0.1) == 17.0);
    REQUIRE(mk::percentile(v, 0.9) == 17.0);
}
