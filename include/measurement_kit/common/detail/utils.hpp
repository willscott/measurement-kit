...
ErrorOr<std::vector<T>> slurpv(const std::string &p) {
...
template <MK_MOCK_AS(std::fopen, std_fopen),
          ...
          MK_MOCK_AS(std::fclose, std_fclose)>
Error overwrite_file(const std::string &path, const std::string &content) {
...
T split(const std::string &s, const std::string &pattern = "\\s+") {
